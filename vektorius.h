...
public:
    ...
    // FIX #2 & #4: resize() now properly destroys truncated elements so that
    // RAII types (string, unique_ptr, file handles, …) release resources
    // immediately rather than waiting until the whole array is deleted.
    // Growth capacity calculation now mirrors push_back() to avoid the
    // talpa_ == 0 edge case producing a zero-capacity allocation.
    void resize(size_t new_size) {
        ...
        // Destroy elements that fall outside the new size
    ...
    void clear() {
        // Destroy all live elements before resetting the size counter
        for (size_t i = 0; i < dydis_; ++i) {
            duomenys_[i] = T{};
        }
        dydis_ = 0;
